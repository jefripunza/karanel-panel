...
module.exports = (option = {}) => {
        ...
        function (err, ip_dns, fam) {
          console.log(
            [
              ...
              `http://104.248.151.102:${config.app.port}`,
            ].join("\n ")
          );
        }
      );
  ...
  const webservers = servers.listen(config.app.ports, config.app.host, () => {
    if (option.debug) {
      require("dns").lookup(
        require("os").hostname(),
        function (err, ip_dns, fam) {
          console.log(
            [
              "Server is running at",
              `https://localhost:${config.app.ports}`,
              `https://${ip_dns}:${config.app.ports}`,
              `https://104.248.151.102:${config.app.ports}`,
            ].join("\n ")
          );
        }
      );
